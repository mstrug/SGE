...
#define SEEK_SET ESeekStart
#define SEEK_CUR ESeekCurrent
#define SEEK_END ESeekEnd

#define strcpy( a, b )
inline int strnlen ( TUint8* a, int b ) { for ( int i = 0; i < b; i++ ) if ( a[ i ] == 0 ) return ( i - 1 ); return b; } 
inline int stricmp( TUint8* a, TUint8* b )
	{	// todo : case insensitive
	for ( int i = 0; a[ i ] != 0 && b[ i ] != 0; i++ ) if ( a[ i ] > b[ i ] ) return 1; else if ( a[ i ] < b[ i ] ) return -1;
	return 0;
	}
inline int strncmp( TInt8* a, TInt8* b, int max )
	{
	for ( int i = 0; i < max; i++ ) if ( a[ i ] > b[ i ] ) return 1; else if ( a[ i ] < b[ i ] ) return -1;
	return 0;
	}
#define malloc( a ) new TUint8[a]
#define free( a ) delete [] a
inline void* realloc( void* a, int size )
	{
	void* tmp = (void*) new TUint8[ size ];
