...
void CMyApplication::InitSGEGl()
    ...
    iCubeData = CGlVertexArray::NewL( cfg, 8 );

    iCubeData->SetVertexDataL( (TAny*) vertices, 8 * 3 * sizeof( GLbyte ) );
    iCubeData->SetColorDataL( (TAny*) colors, 8 * 4 * sizeof( GLubyte ) );
    iCubeData->SetIndicesL( (TAny*) triangles, 12 * 3 * sizeof( GLubyte ), EFalse );
...
void CMyApplication::DrawBox( GLfloat aSizeX, GLfloat aSizeY, GLfloat aSizeZ )
    ...
    iCubeData->SetColorArrayEnabled( ETrue );
    iCube->Draw( GL_TRIANGLES );
    
    iCubeData->SetColorArrayEnabled( EFalse );
